#[cfg(feature = "librustzcash_zebra_compatibility")]
pub mod librustzcash_zebra_compatibility {
    ...
    pub fn try_from_configuredactivationheights_to_localnetwork(
        ...
        if let Some(nu7_height) = nu7 {
            return Err(
                TryFromConfiguredActivationHeightsToLocalNetworkError::Unsupported(
                    UnsupportedProtocolActivationSpecification::NU7(BlockHeight::from_u32(
                        nu7_height,
                    )),
                ),
            );
