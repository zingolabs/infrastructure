...
fn pick_path(executable_name: &str) -> Option<PathBuf> {
    ...
    match std::env::var(environment_variable_path) {
        Ok(directory) => {
            ...
            if path.exists() {
                tklog::info!("Running {executable_name} at {path:?}.");
                Some(path)
            } else {
                tklog::warn!("Could not find {executable_name} at {path:?} set by {environment_variable_path}.");
                None
            }
        }
        Err(_err) => {
            tklog::warn!("{environment_variable_path} environment variable is not set. Will attempt to use {executable_name} from PATH.");
            None
        }
    }
}
